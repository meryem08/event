...
const EventsManagers = () => {
  const [eventManagers, setEventManagers] = useState(data);
  const [searchQuery, setSearchQuery] = useState("");

  const handleDelete = (id) => {
    const updatedEventManagers = eventManagers.filter((eventsManager) => eventsManager.id !== id);
  ...
  const filteredEventManagers = eventManagers.filter((eventsManager) =>
    eventsManager.name.first.toLowerCase().includes(searchQuery.toLowerCase()) ||
    eventsManager.name.last.toLowerCase().includes(searchQuery.toLowerCase())
  );
            ...
            {filteredEventManagers.map((eventsManager) => (
              ...
              >
                ...
                <p className="text-gray-600 sm:text-left text-right">
                  {eventsManager.name.first + " " + eventsManager.name.last}
                </p>
                <p className="hidden md:flex">{eventsManager.email}</p>
                <div className="sm:flex hidden justify-between items-center">
                  <DeleteButton onClick={() => confirmDelete(eventsManager.id)} />
                  <BsThreeDotsVertical />
                </div>
              </li>
            ))}
